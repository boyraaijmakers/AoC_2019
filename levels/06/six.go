...
func Six() {
	...
	inputOne := strings.Split(wireOne, ",")
	inputTwo := strings.Split(wireTwo, ",")
	var grid [10000][10000]int32

	addWire(&grid, inputOne, false)
	addWire(&grid, inputTwo, true)
}

func addWire(grid *[10000][10000]int32, wire []string, secondWire bool) {
	currentPosition := [2]int32{5000, 5000}

	var wireSteps int32 = 0

	for _, rule := range wire {

		steps, _ := strconv.Atoi(rule[1:])
		switch direction := string(rule[0]); direction {
		case "R":
			moveCell(grid, &currentPosition, steps, [4]int32{0, 1, 0, 0}, &wireSteps, secondWire)
		case "L":
			moveCell(grid, &currentPosition, steps, [4]int32{0, 0, 0, 1}, &wireSteps, secondWire)
		case "U":
			moveCell(grid, &currentPosition, steps, [4]int32{1, 0, 0, 0}, &wireSteps, secondWire)
		case "D":
			moveCell(grid, &currentPosition, steps, [4]int32{0, 0, 1, 0}, &wireSteps, secondWire)
		}

	}
}

func moveCell(grid *[10000][10000]int32, pointer *[2]int32, steps int, delta [4]int32, wireSteps *int32, secondWire bool) {
	for i := 0; i < steps; i++ {
		pointer[0] += delta[0] - delta[2]
		pointer[1] += delta[1] - delta[3]
		*wireSteps++
		if pointer[0] >= 0 && pointer[0] < 10000 && pointer[1] >= 0 && pointer[1] < 10000 {
			if grid[pointer[0]][pointer[1]] > 0 && secondWire {
				println("Answer!")
				println(grid[pointer[0]][pointer[1]] + *wireSteps)
			}
			grid[pointer[0]][pointer[1]] += *wireSteps
		}
	}
}
