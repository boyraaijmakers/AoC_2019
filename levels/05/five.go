...
func Five() {
	...
	var grid [10000][30000]int8

	addWire(&grid, inputOne)
	addWire(&grid, inputTwo)
	...
	for y, row := range grid {
		for x, cell := range row {
			if cell > 1 {
				fmt.Printf("(%v, %v)", x, y)
				distance := math.Abs(float64(x-15000)) + math.Abs(float64(y-6000))
				if distance < minDist {
					minDist = distance
				}
			}
...
func addWire(grid *[10000][30000]int8, wire []string) {
	currentPosition := [2]int64{6000, 15000}
	...
	for _, rule := range wire {
		...
		case "R":
			moveCell(grid, &currentPosition, steps, [4]int64{0, 1, 0, 0})
		case "L":
			moveCell(grid, &currentPosition, steps, [4]int64{0, 0, 0, 1})
		case "U":
			moveCell(grid, &currentPosition, steps, [4]int64{1, 0, 0, 0})
		case "D":
			moveCell(grid, &currentPosition, steps, [4]int64{0, 0, 1, 0})
		}

	}
}

func moveCell(grid *[10000][30000]int8, pointer *[2]int64, steps int, delta [4]int64) {
	for i := 0; i < steps; i++ {
		pointer[0] += delta[0] - delta[2]
		pointer[1] += delta[1] - delta[3]

		if pointer[0] >= 0 && pointer[0] < 10000 && pointer[1] >= 0 && pointer[1] < 30000 {
